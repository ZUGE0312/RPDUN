...
def generate_masks(mask_path, batch_size):
    mask = sio.loadmat(mask_path + '/mask_simu.mat')
    mask = mask['mask']
...
def LoadTest(path):
    ...
    imgs = []
    for i, data_name in enumerate(train_list):
        ...
        elif 'img_expand' in data:
            data = data['img_expand']

        imgs.append(data)
        print(f"Scene {i+1} is loaded. {data_name}")
...
def shift_dataset(HSI, mask, step=2):       # [28,256,256] -> [256,310]
    c, h, w = HSI.shape
    if mask is None:
        temp = HSI
    else:
        temp = HSI * mask
    meas = torch.zeros(c, h, w+(c-1)*step).float()
    for i in range(c):
        meas[i, :, i*step:i*step+w] = temp[i, :, :]
    meas = torch.sum(meas, 0)       # 256,310
